...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/lux.conf')));
var LuxService = proxyquire('../../lib/services/luxd', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultLuxConf = fs.readFileSync(path.resolve(__dirname, '../data/default.lux.conf'), 'utf8');

describe('Lux Service', function() {
	...
	var baseConfig = {
		node: {
			network: luxcore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		...
		it('will create an instance without `new`', function() {
			var luxd = LuxService(baseConfig);
			should.exist(luxd);
		});
		it('will init caches', function() {
			var luxd = new LuxService(baseConfig);

			should.exist(luxd.utxosCache);
			should.exist(luxd.txidsCache);
			should.exist(luxd.balanceCache);
			should.exist(luxd.summaryCache);
			should.exist(luxd.transactionDetailedCache);
			should.exist(luxd.accountInfo);

			should.exist(luxd.transactionCache);
			should.exist(luxd.rawTransactionCache);
			should.exist(luxd.rawJsonTransactionCache);
			should.exist(luxd.blockCache);
			should.exist(luxd.blockJsonCache);
			should.exist(luxd.blockSubsidyCache);
			should.exist(luxd.rawBlockCache);
			should.exist(luxd.blockHeaderCache);
			should.exist(luxd.zmqKnownTransactions);
			should.exist(luxd.zmqKnownBlocks);
			should.exist(luxd.lastTip);
			should.exist(luxd.lastTipTimeout);
			should.equal(luxd.dgpInfoCache, null, 'should be null');
			should.equal(luxd.miningInfoCache, null, 'should be null');
			should.equal(luxd.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(luxd.maxTxids, 1000);
			should.equal(luxd.maxTransactionHistory, 50);
			should.equal(luxd.maxAddressesQuery, 10000);
			should.equal(luxd.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(luxd.spawnRestartTime, 5000);
			should.equal(luxd.spawnStopTime, 10000);

			// try all interval
			should.equal(luxd.tryAllInterval, 1000);
			should.equal(luxd.startRetryInterval, 5000);

			// rpc limits
			should.equal(luxd.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(luxd.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var luxd = new LuxService(baseConfig);
			luxd.nodes.should.deep.equal([]);
			luxd.nodesIndex.should.equal(0);
			luxd.nodes.push({ client: sinon.stub() });
			should.exist(luxd.client);
		});
		it('will set subscriptions', function() {
			var luxd = new LuxService(baseConfig);
			luxd.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var luxd = new LuxService(baseConfig);
			luxd._initDefaults({ transactionConcurrency: 10 });
			luxd.transactionConcurrency.should.equal(10);
			luxd._initDefaults({});
			luxd.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			LuxService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var luxd = new LuxService(baseConfig);
			var methods = luxd.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var luxd = new LuxService(baseConfig);
			var events = luxd.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('luxd/rawtransaction');
			events[0].scope.should.equal(luxd);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('luxd/hashblock');
			events[1].scope.should.equal(luxd);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('luxd/addresstxid');
			events[2].scope.should.equal(luxd);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('luxd/addressbalance');
			events[3].scope.should.equal(luxd);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var luxd = new LuxService(baseConfig);
			luxd.subscribe = sinon.stub();
			luxd.unsubscribe = sinon.stub();
			var events = luxd.getPublishEvents();

			events[0].subscribe('test');
			luxd.subscribe.args[0][0].should.equal('rawtransaction');
			luxd.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			luxd.unsubscribe.args[0][0].should.equal('rawtransaction');
			luxd.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			luxd.subscribe.args[1][0].should.equal('hashblock');
			luxd.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			luxd.unsubscribe.args[1][0].should.equal('hashblock');
			luxd.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var luxd = new LuxService(baseConfig);
			var emitter = {};
			luxd.subscribe('hashblock', emitter);
			luxd.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			luxd.subscribe('rawtransaction', emitter2);
			luxd.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			luxd.subscribe('hashblock', emitter1);
			luxd.subscribe('hashblock', emitter2);
			luxd.subscribe('hashblock', emitter3);
			luxd.subscribe('hashblock', emitter4);
			luxd.subscribe('hashblock', emitter5);
			luxd.subscriptions.hashblock.length.should.equal(5);

			luxd.unsubscribe('hashblock', emitter3);
			luxd.subscriptions.hashblock.length.should.equal(4);
			luxd.subscriptions.hashblock[0].should.equal(emitter1);
			luxd.subscriptions.hashblock[1].should.equal(emitter2);
			luxd.subscriptions.hashblock[2].should.equal(emitter4);
			luxd.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var luxd = new LuxService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			luxd.subscriptions.hashblock = [emitter1];
			luxd.unsubscribe('hashblock', emitter3);
			luxd.subscriptions.hashblock.length.should.equal(1);
			luxd.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			luxd.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(luxd.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var luxd = new LuxService(baseConfig);
			var emitter = new EventEmitter();
			luxd.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var luxd = new LuxService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			luxd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			luxd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			luxd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			luxd.unsubscribeAddress(emitter1);
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			luxd.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			luxd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			luxd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			luxd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(luxd, 'unsubscribeAddressAll');
			luxd.unsubscribeAddress(emitter1);
			luxd.unsubscribeAddressAll.callCount.should.equal(1);
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			luxd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			luxd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			luxd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			luxd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			luxd.unsubscribeAddressAll(emitter1);
			luxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			luxd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			luxd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(luxd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var luxd = new LuxService(baseConfig);
			var config = luxd._getDefaultConfig();
			config.should.equal(defaultLuxConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a lux.conf file', function() {
			var TestLux = proxyquire('../../lib/services/luxd', {
				fs: {
					readFileSync: readFileSync,
			...
			var luxd = new TestLux(baseConfig);
			luxd.options.spawn.datadir = '/tmp/.lux';

			sinon.spy(luxd, '_expandRelativeDatadir');
			sinon.spy(luxd, '_checkConfigIndexes');
			sinon.spy(luxd, '_getDefaultConf')
			sinon.spy(luxd, '_getNetworkConfigPath');
			sinon.spy(luxd, '_parseBitcoinConf');

			var node = {};
			luxd._loadSpawnConfiguration(node);

			luxd._expandRelativeDatadir.callCount.should.equal(1);
			luxd._checkConfigIndexes.callCount.should.equal(1);
			luxd._getDefaultConf.callCount.should.equal(1);
			luxd._getNetworkConfigPath.callCount.should.equal(1);
			luxd._parseBitcoinConf.callCount.should.equal(2);

			should.exist(luxd.spawn.config);
			luxd.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: luxcore.Networks.testnet,
					configPath: '/tmp/.luxcore/luxcore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			luxd.options.spawn.datadir = './data';
			var node = {};
			luxd._loadSpawnConfiguration(node);
			...
			luxd.options.spawn.datadir.should.equal('/tmp/.luxcore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestLux = proxyquire('../../lib/services/luxd', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badlux.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var luxd = new TestLux(baseConfig);
			(function() {
				luxd._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(luxcore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the lux.conf file', function() {
			var luxd = new LuxService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				luxd._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the lux.conf file', function() {
			var luxd = new LuxService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the lux.conf file', function() {
			var luxd = new LuxService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the lux.conf file', function() {
			var luxd = new LuxService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the lux.conf file', function() {
			var luxd = new LuxService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				luxd.transactionDetailedCache.set(keys[i], {});
				luxd.utxosCache.set(keys[i], {});
				luxd.accountInfo.set(keys[i], {});
				luxd.txidsCache.set(keys[i], {});
				luxd.balanceCache.set(keys[i], {});
				luxd.summaryCache.set(keys[i], {});
				luxd.blockOverviewCache.set(keys[i], {});
				luxd.blockJsonCache.set(keys[i], {});
			}
			luxd._resetCaches();
			should.equal(luxd.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(luxd.utxosCache.get(keys[0]), undefined);
			should.equal(luxd.accountInfo.get(keys[0]), undefined);
			should.equal(luxd.txidsCache.get(keys[0]), undefined);
			should.equal(luxd.balanceCache.get(keys[0]), undefined);
			should.equal(luxd.summaryCache.get(keys[0]), undefined);
			should.equal(luxd.blockOverviewCache.get(keys[0]), undefined);
			should.equal(luxd.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			luxd._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				luxd.nodes[0].client.getInfo.callCount.should.equal(1);
				luxd.nodes[1].client.getInfo.callCount.should.equal(1);
				luxd.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			luxd._tryAllClients(function(client, next) {
				...
				luxd.nodes[1].client.getInfo.callCount.should.equal(1);
				luxd.nodes[2].client.getInfo.callCount.should.equal(1);
				luxd.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert luxd-rpc error object into JavaScript error', function() {
			var luxd = new LuxService(baseConfig);
			var error = luxd._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			luxd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			luxd.nodes.push({
				client: {
			...
			luxd._initChain(function() {
				log.info.callCount.should.equal(1);
				luxd.getRawBlock.callCount.should.equal(1);
				luxd.getRawBlock.args[0][0].should.equal('genesishash');
				luxd.height.should.equal(5000);
				luxd.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			luxd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			luxd._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			luxcore.Networks.disableRegtest();
			baseConfig.node.network = luxcore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: luxcore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var luxd = new LuxService(config);
			should.equal(luxd._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var luxd = new LuxService(config);
			luxd._getNetworkConfigPath().should.equal('testnet3/lux.conf');
		});
		it('will get default rpc port for regtest', function() {
			luxcore.Networks.enableRegtest();
			var config = {
			...
			var luxd = new LuxService(config);
			luxd._getNetworkConfigPath().should.equal('regtest/lux.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var luxd = new LuxService(baseConfig);
			luxd.node.network = luxcore.Networks.testnet;
			luxd._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var luxd = new LuxService(baseConfig);
			luxd.node.network = luxcore.Networks.testnet;
			luxcore.Networks.enableRegtest();
			luxd._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var luxd = new LuxService(baseConfig);
			luxd.node.network = luxcore.Networks.livenet;
			luxcore.Networks.enableRegtest();
			should.equal(luxd._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			luxd._rapidProtectedUpdateTip = sinon.stub();
			luxd.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			luxd._zmqBlockHandler(node, message);
			luxd._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			luxd.on('block', function(block) {
				block.should.equal(message);
				luxd.emit.callCount.should.equal(1);
				luxd._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			luxd._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			luxd._zmqBlockHandler(node, message);
			luxd._rapidProtectedUpdateTip.callCount.should.equal(1);
			luxd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			luxd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(luxd, 'emit');
			var emitter = new EventEmitter();
			luxd.subscriptions.hashblock.push(emitter);
			emitter.on('luxd/hashblock', function(blockHash) {
				luxd._rapidProtectedUpdateTip.callCount.should.equal(1);
				luxd.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				luxd._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.syncPercentage = sinon.stub();
			sinon.spy(luxd, '_resetCaches');
			luxd.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				luxd._resetCaches.callCount.should.equal(1);
			...
			luxd._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			luxd.on('synced', function() {
				luxd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			luxd.on('synced', function() {
				luxd.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			luxd.on('error', function(err) {
				log.error.callCount.should.equal(1);
				luxd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.syncPercentage = sinon.stub();
			luxd._resetCaches = sinon.stub();
			luxd.on('tip', function(height) {
				luxd._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				luxd.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using luxcore.Transaction', function() {
			var luxd = new LuxService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = luxcore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(luxcore.Networks.testnet);
			var outputAddress = luxcore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = luxcore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: luxcore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = luxd._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addOutput(luxcore.Transaction.Output({
				script: luxcore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = luxd._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var luxd = new LuxService(baseConfig);
			var tx = luxcore.Transaction();
			var address = luxcore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(luxcore.Transaction.Output({
				script: luxcore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(luxcore.Transaction.Output({
				script: luxcore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			luxd._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			luxd.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('luxd/addresstxid', function(data) {
			...
			luxd._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			luxd.subscriptions.rawtransaction.push(emitter);
			emitter.on('luxd/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			luxd._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(luxd, 'emit');
			luxd.subscriptions.rawtransaction.push(emitter);
			emitter.on('luxd/rawtransaction', function() {
				done();
			});
			var node = {};
			luxd._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd._updateTip = sinon.stub();
			luxd._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			luxd.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			luxd._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				luxd._updateTip.callCount.should.equal(11);
				luxd._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				luxd._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			luxd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				luxd._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			luxd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				luxd._zmqBlockHandler.callCount.should.equal(0);
				luxd._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var luxd = new LuxService(baseConfig);
			var node = {};
			luxd._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			luxd._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			luxd._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			luxd._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestLuxService = proxyquire('../../lib/services/luxd', {
				fs: {
					readFile: readFile
				}
			});
			var luxd = new TestLuxService(baseConfig);
			luxd.spawnStopTime = 1;
			luxd._process = {};
			luxd._process.kill = sinon.stub();
			luxd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				luxd._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			luxd._process.kill = sinon.stub().throws(error2);
			luxd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd._loadSpawnConfiguration = sinon.stub();
			luxd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			luxd._spawnChildProcess(function(err) {
				luxd._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var luxd = new LuxService(baseConfig);
			luxd._loadSpawnConfiguration = sinon.stub();
			luxd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			luxd._spawnChildProcess(function(err) {
				luxd._loadSpawnConfiguration.callCount.should.equal(1);
				luxd._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var luxd = new TestLuxService(config);
			luxd.spawn = {};
			luxd._loadSpawnConfiguration = sinon.stub();
			luxd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			luxd.node.stopping = true;
			luxd._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			luxd._loadSpawnConfiguration = sinon.stub();
			luxd.spawn = {};
			luxd.spawn.exec = 'testexec';
			luxd.spawn.configPath = 'testdir/lux.conf';
			luxd.spawn.datadir = 'testdir';
			luxd.spawn.config = {};
			luxd.spawn.config.rpcport = 20001;
			luxd.spawn.config.rpcuser = 'lux';
			luxd.spawn.config.rpcpassword = 'password';
			luxd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			luxd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			luxd._initZmqSubSocket = sinon.stub();
			luxd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			luxd._checkReindex = sinon.stub().callsArgWith(1, null);
			luxd._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/lux.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				luxd._loadTipFromNode.callCount.should.equal(1);
				luxd._initZmqSubSocket.callCount.should.equal(1);
				should.exist(luxd._initZmqSubSocket.args[0][0].client);
				luxd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				luxd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(luxd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn luxd spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestLuxService = proxyquire('../../lib/services/luxd', {
				fs: {
			...
			luxd._loadSpawnConfiguration = sinon.stub();
			luxd.spawn = {};
			luxd.spawn.exec = 'luxd';
			luxd.spawn.datadir = '/tmp/lux';
			luxd.spawn.configPath = '/tmp/lux/lux.conf';
			luxd.spawn.config = {};
			luxd.spawnRestartTime = 1;
			luxd._loadTipFromNode = sinon.stub().callsArg(1);
			luxd._initZmqSubSocket = sinon.stub();
			luxd._checkReindex = sinon.stub().callsArg(1);
			luxd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			luxd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(luxd, '_spawnChildProcess');
			luxd._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						luxd._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			luxd._spawnChildProcess(function(err) {
				...
				luxd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				luxd.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn luxd spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestLuxService = proxyquire('../../lib/services/luxd', {
				fs: {
			...
			luxd._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						luxd._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			luxd.spawn.config.rpcpassword = 'password';
			luxd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			luxd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			luxd._spawnChildProcess(function(err) {
				luxd._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			luxd._initZmqSubSocket = sinon.stub();
			luxd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			luxd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var luxd = new LuxService(config);
			luxd.node.stopping = true;
			luxd.startRetryInterval = 100;
			luxd._loadTipFromNode = sinon.stub();
			luxd._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to luxd/);
				luxd._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			luxd.startRetryInterval = 1;
			var config = {};
			luxd._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				luxd._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			luxd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			luxd._connectProcess(config, function(err, node) {
				should.not.exist(err);
				luxd._loadTipFromNode.callCount.should.equal(1);
				luxd._initZmqSubSocket.callCount.should.equal(1);
				luxd._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			...
			luxd.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('Lux configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			luxd.options = {
				connect: [
					{}
				]
			...
			luxd.start(function(err) {
				luxd._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var luxd = new LuxService(baseConfig);
			var node = {};
			luxd._initChain = sinon.stub().callsArg(0);
			luxd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			luxd.options = {
				spawn: {}
			};
			luxd.start(function(err) {
				should.not.exist(err);
				luxd.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(luxd, '_spawnChildProcess');
			var nodes = [{}];
			luxd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			luxd.options = {
				connect: [
					{}
				]
			...
			luxd.start(function(err) {
				should.not.exist(err);
				luxd._initChain.callCount.should.equal(1);
				luxd._connectProcess.callCount.should.equal(1);
				luxd.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			luxd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				luxd.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			luxd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			luxd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			luxd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			luxd.syncPercentage(function(err) {
				should.exist(err);
				luxd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			luxd.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				luxd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var luxd = new LuxService(baseConfig);
			var args = luxd._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var luxd = new LuxService(baseConfig);
			var args = luxd._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var luxd = new LuxService(baseConfig);
			sinon.spy(luxd, '_normalizeAddressArg');
			luxd.nodes.push({
				client: {
			...
			var options = {};
			luxd.getAddressBalance(address, options, function(err) {
				luxd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			luxd.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				luxd.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					luxd._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			luxd.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = luxd._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = luxd._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = luxd._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = luxd._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				luxd._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			luxd.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(luxd, '_getHeightRangeQuery');
			sinon.spy(luxd, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			luxd.getAddressTxids(address, options, function(err) {
				should.exist(err);
				luxd._getHeightRangeQuery.callCount.should.equal(1);
				luxd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			luxd.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				luxd._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			luxd.getAddressTxids(address, options, function(err, txids) {
				...
				luxd.getAddressTxids(address, options, function(err, txids) {
					...
					luxd._getHeightRangeQuery.callCount.should.equal(2);
					luxd._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			luxd.getAddressTxids(address, options, function(err, txids) {
				...
				luxd.getAddressTxids(address, options, function(err, txids) {
					...
					luxd._getHeightRangeQuery.callCount.should.equal(4);
					luxd._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			luxd.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				luxd.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					luxd.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						luxd._getHeightRangeQuery.callCount.should.equal(3);
						luxd._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var luxd = new LuxService(baseConfig);
			luxd.height = 10;
			var confirmations = luxd._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var luxd = new LuxService(baseConfig);
			var result = {};
			luxd._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var luxd = new LuxService(baseConfig);
			var result = {};
			luxd._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var luxd = new LuxService(baseConfig);
			var result = {};
			luxd._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var luxd = new LuxService(baseConfig);
			var result = {};
			luxd._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(luxd, '_getAddressDetailsForInput');
			sinon.spy(luxd, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = luxd._getAddressDetailsForTransaction(tx, addresses);
			...
			luxd._getAddressDetailsForInput.callCount.should.equal(1);
			luxd._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var luxd = new LuxService(baseConfig);
			sinon.spy(luxd, '_getConfirmationsDetail');
			luxd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			luxd.height = 300;
			var addresses = {};
			luxd._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			luxd._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				luxd.getDetailedTransaction.callCount.should.equal(1);
				luxd._getAddressDetailsForTransaction.callCount.should.equal(1);
				luxd._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var luxd = new LuxService(baseConfig);
			luxd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			luxd._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				luxd.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from luxcore addresses', function() {
			var addresses = [
				luxcore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				luxcore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var luxd = new LuxService(baseConfig);
			var strings = luxd._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var luxd = new LuxService(baseConfig);
			(function() {
				luxd._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var luxd = new LuxService(baseConfig);
			sinon.spy(luxd, '_normalizeAddressArg');
			luxd.maxAddressesQuery = 100;
			luxd.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				luxd._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var luxd = new LuxService(baseConfig);
			sinon.spy(luxd, '_normalizeAddressArg');
			sinon.spy(luxd, '_getAddressStrings');
			luxd.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				luxd._normalizeAddressArg.callCount.should.equal(1);
				luxd._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(luxd, '_normalizeAddressArg');
			sinon.spy(luxd, '_getAddressStrings');
			luxd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			luxd.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				luxd._normalizeAddressArg.callCount.should.equal(1);
				luxd._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(luxd, '_normalizeAddressArg');
			sinon.spy(luxd, '_getAddressStrings');
			luxd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			luxd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			luxd.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				luxd._normalizeAddressArg.callCount.should.equal(1);
				luxd._getAddressStrings.callCount.should.equal(1);
				luxd.getAddressTxids.callCount.should.equal(1);
				luxd._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(luxd, '_normalizeAddressArg');
			sinon.spy(luxd, '_getAddressStrings');
			sinon.spy(luxd, 'getAddressTxids');
			luxd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			luxd.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				luxd._normalizeAddressArg.callCount.should.equal(1);
				luxd._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(luxd, '_normalizeAddressArg');
			sinon.spy(luxd, '_getAddressStrings');
			sinon.spy(luxd, '_paginateTxids');
			luxd._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(luxd, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			luxd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			luxd.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				luxd._normalizeAddressArg.callCount.should.equal(1);
				luxd._getAddressStrings.callCount.should.equal(1);
				luxd.getAddressTxids.callCount.should.equal(1);
				luxd._getAddressDetailedTransaction.callCount.should.equal(2);
				luxd._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			luxd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			luxd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			luxd.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			luxd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			luxd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			luxd.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			luxd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			luxd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			luxd.getAddressSummary(address, options, function(err, summary) {
				luxd._paginateTxids.callCount.should.equal(1);
				luxd._normalizeAddressArg.callCount.should.equal(1);
				luxd.getAddressTxids.callCount.should.equal(1);
				luxd.getAddressBalance.callCount.should.equal(1);
				luxd._paginateTxids.args[0][1].should.equal(0);
				luxd._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			luxd.getAddressSummary(address, options, function(err, summary) {
				...
				luxd.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					luxd._paginateTxids.callCount.should.equal(0);
					luxd._normalizeAddressArg.callCount.should.equal(2);
					luxd.getAddressTxids.callCount.should.equal(1);
					luxd.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			luxd.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				luxd._paginateTxids.callCount.should.equal(1);
				luxd._normalizeAddressArg.callCount.should.equal(1);
				luxd.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			luxd._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			luxd.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			luxd.getRawBlock(blockhash, function(err) {
				should.exist(err);
		...
		it('will try all nodes for getblock', function(done) {
			...
			luxd.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			luxd.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			luxd.getBlock(0, function(err) {
				luxd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as luxcore object from height', function(done) {
			var luxd = new LuxService(baseConfig);
			sinon.spy(luxd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			luxd.getBlock(0, function(err, block) {
				should.not.exist(err);
				luxd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(luxcore.Block);
				done();
			});
		});
		it('will getblock as luxcore object', function(done) {
			var luxd = new LuxService(baseConfig);
			sinon.spy(luxd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			luxd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				luxd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			luxd.getBlock(0, function(err, block) {
				...
				luxd.getBlock(0, function(err, block) {
					should.not.exist(err);
					luxd._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			luxd.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var luxd = new LuxService(baseConfig);
			sinon.spy(luxd, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			luxd.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			luxd.getBlockHeader(10, function(err) {
				luxd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			luxd.getBlockHeader(blockhash, function(err) {
				luxd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			luxd.getBlockHeader(0, function(err) {
				luxd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			luxd.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				luxd._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			luxd.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				luxd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			luxd._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				luxd._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			luxd._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			luxd._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var luxd = new LuxService(baseConfig);
			sinon.spy(luxd, '_maybeGetBlockHash');
			luxd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			luxd.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				luxd._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			luxd.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				luxd.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					luxd._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			luxd.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			luxd.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = luxcore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			luxd.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			luxd.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			luxd.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = luxcore.Transaction();
			(function() {
				luxd.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getRawTransaction('txid', function(err) {
				should.exist(err);
				luxd._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			luxd.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getTransaction('txid', function(err) {
				should.exist(err);
				luxd._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			luxd.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				luxd._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(luxcore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			luxd.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				luxd._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			luxd.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			luxd.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			luxd.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.node.getNetworkName = sinon.stub().returns('testnet');
			...
			luxd.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			luxd.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			luxd.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				luxd.getSubsidy.callCount.should.equal(1);
				luxd.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			luxd.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			luxd.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.spawn = {};
			luxd.spawn.process = new EventEmitter();
			luxd.spawn.process.kill = sinon.stub();
			luxd.stop(done);
			luxd.spawn.process.kill.callCount.should.equal(1);
			luxd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			luxd.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			luxd.spawn.process.kill.callCount.should.equal(1);
			luxd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			luxd.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var luxd = new LuxService(baseConfig);
			luxd.shutdownTimeout = 300;
			luxd.spawn = {};
			luxd.spawn.process = new EventEmitter();
			luxd.spawn.process.kill = sinon.stub();
			luxd.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				luxd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var luxd = new LuxService(baseConfig);
			sinon.spy(luxd, '_normalizeAddressArg');
			sinon.spy(luxd, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			luxd.nodes.push({
			...
			luxd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				luxd._getBalanceFromMempool.callCount.should.equal(1);
				luxd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				luxd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			luxd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				luxd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			luxd.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			luxd.getJsonBlock(0, function(err) {
				should.exist(err);
				luxd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			luxd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				luxd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			luxd.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				luxd._tryAllClients.callCount.should.equal(2);
				luxd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			luxd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				luxd._tryAllClients.callCount.should.equal(1);
				luxd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var luxd = new LuxService(baseConfig);
			var emitter = new EventEmitter();
			luxd.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(luxd.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var luxd = new LuxService(baseConfig);
			var emitter = new EventEmitter();
			luxd.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			luxd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			luxd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			luxd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			luxd.unsubscribeBalance(emitter1);
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			luxd.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			luxd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var luxd = new LuxService(baseConfig);
			...
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			luxd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			luxd.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var luxd = new LuxService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			luxd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(luxd, 'unsubscribeBalanceAll');
			luxd.unsubscribeBalance(emitter1);
			luxd.unsubscribeBalanceAll.callCount.should.equal(1);
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			luxd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			luxd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			luxd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			luxd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			luxd.unsubscribeBalanceAll(emitter1);
			luxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			luxd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			luxd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(luxd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var luxd = new LuxService(baseConfig);
			sinon.spy(luxd, '_notifyBalanceSubscriber');
			luxd._getAddressesFromTransaction = sinon.stub().returns(addresses);
			luxd.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('luxd/addressbalance', function(data) {
				...
				luxd._notifyBalanceSubscriber.callCount.should.equal(1);
				luxd._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			luxd._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('luxd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				luxd.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			luxd._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			luxd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			luxd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			luxd.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				luxd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getDgpInfo(function(err, result) {
				should.exist(err);
				luxd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			luxd.getMiningInfo(function(err, result) {
				should.exist(err);
				luxd._tryAllClients.callCount.should.equal(1);
