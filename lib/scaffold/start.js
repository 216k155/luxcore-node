...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for luxd is necessary for this upgrade with the "reindex=1" lux.conf option. \n' +
                 'There are changes necessary in both lux.conf and luxcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed luxcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './luxcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in luxcore-node services directory
    ...
    if (servicePackage.luxcoreNode) {
      serviceModule = service.name + '/' + servicePackage.luxcoreNode;
    }
