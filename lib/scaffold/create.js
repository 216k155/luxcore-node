...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'luxcore-lib': 'https://github.com/216k155/luxcore-lib.git#master',
    'luxcore-node': 'https://github.com/216k155/luxcore-node.git#master'
  }
};

/**
 * Will create a directory and lux.conf file for Bitcoin.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/luxcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the luxcore-node directory and configuration
