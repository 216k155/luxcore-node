...
Lux.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getStakingStatus', this, this.getStakingStatus, 0]
  ];
  return methods;
};
...
Lux.prototype.subscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'luxd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};
...
Lux.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'luxd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};
    ...
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('luxd/addressbalance', {
                    address: address,
                    txid: txid,
 ...
 * @param {Array} addresses - A array of lux addresses to filter
 * @param {Function} callback
 */
...
Lux.prototype.getSubsidy = function(height, callback) {
    ...
    self.blockSubsidyCache.set(height, 0.8);
    callback(null, 0.8);
};
 ...
 * @return {*}
 */
Lux.prototype.getStakingStatus = function(callback) {
    var self = this,
    ...
    return self._tryAllClients(function(client, done) {
        return client.getStakingStatus(function(err, response) {
            if (err) {
                return done(self._wrapRPCError(err));
            }
